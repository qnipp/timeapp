...
const year = moment().locale('en').format('YYYY');
for (index = 0; index < 12; index++) {
  const month = moment().locale('en').month(index).format('MMMM');
  CNF.timeslots[`${month}_${year}`] = {
    label: month,
    usage: 'report',
    start: {
      $gte: moment().year(year).month(index).startOf('month').toDate(),
      $lte: moment().year(year).month(index).endOf('month').toDate(),
    },
  };
}
...
CNF.timeslots[`Total ${year-1}`] = {
  ...
  usage: 'report',
  start: {
    $gte: moment().year(year-1).startOf('year').toDate(),
    $lte: moment().year(year-1).endOf('year').toDate(),
  },
};
