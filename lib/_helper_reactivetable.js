...
tableSettingsPerMonth = function () {
	...
	for (index = 0; index < 11; index++) {
		var month = moment().month(index).format("MMMM");
		settings.fields.push({
			key: 'time'+month, 
			label: month, 
			fn: function(value, item, key) {
				var monthname = key.replace(/time/,'');
				//console.log('in fn: '+ month + ' monthname: '+ monthname);
				
				return formatDuration(queryTimes(
					moment().month(monthname).startOf("month").toDate(), 
					moment().month(monthname).endOf("month").toDate(), 
					null, null, Meteor.userId()), true);
			},
			sortable: false,
		});
	}
