      ...
      },
      {
        ...
        fn(value, object, key) {
          if(moment(value, CNF.FORMAT_DATETIME_SORT).isValid()) {
            return moment(value, CNF.FORMAT_DATETIME_SORT).format(CNF.FORMAT_TIME);
          } else {
            return value;
          } 
        },
        ...
        fn(value, object, key) {
          if(value && value.length > 0) {
            ...
            return value.reduce(function(prev, newitem) {
              return prev + (prev?" | ":'') +  moment(newitem.start).format(CNF.FORMAT_TIME) + " - " +  moment(newitem.end).format(CNF.FORMAT_TIME)
            }, "");
          }
        },
        // sortByValue: true,
      ...
      },
      {
        ...
        fn(value, object, key) {
          if(object.daysoff && object.daysoff.length > 0) {
            // return value.join(" | ");
            return object.daysoff.length + " day"+ (object.daysoff.length > 1 ? 's' : '');
          }
        },
        // sortByValue: true,
        sortable: true,
      },
      {
        ...
        fn(value, object, key) {
          if(value && value.length > 0) {
            // return value.join(" | ");
            return value.reduce(function(prev, newitem) {
              return prev + (prev?", ":'') + moment(newitem).format(CNF.FORMAT_DATE_SHORT);
            }, "");
          }
        },
        // sortByValue: true,
