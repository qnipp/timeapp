	...
	title: {
		...
		custom: function () {
			if (Meteor.isClient && this.isSet) {
				...
				Meteor.call("itemIsTitleUnique", this.value, this.docId, function (error, result) {
					if (!result) {
						Items.simpleSchema().namedContext("formItem").addInvalidKeys(
	...
	ownedBy:{
		...
		autoValue: function() {
			...
			*/
			...
			if (!this.isSet && !this.value) {
				...
				// .. set current user
				console.log('set userid for new (not existing) entry: ' + Meteor.userId());
				return Meteor.userId();
				
			} else if(this.isSet && this.value != Meteor.userId()) {
				// if current value is set, and is not the current user, unset given val
				
				console.log('updating item to a new owner:  ' + Meteor.userId() + ' > ' + this.value);
				//this.unset();
			}
		},
		...
		custom: function() {
			...
			*/
			...
			if ( this.isSet && this.isUpdate && currentRow.ownedBy && this.userId != currentRow.ownedBy) {
				return "unauthorized";
			}
...
}, Schemas.timeSlots, Schemas.createdFields 
]);
...
Schemas.Times = new SimpleSchema([{
	item: {
		...
		autoform: {
			...
			options: function()  { 
				return Items.find({}).map( 
					function(item) { 
						return { label: item.title + ' - ' + item.description, value: item._id } 
					}
				)
			}
	...
	"comments.$.createdAt": {
		...
		//defaultValue: new Date(),
		autoValue: function() {
			console.log("createdAt autovalue Times: ");
			console.log(this);
			if(this.isInsert) {
				return new Date();
			}
	...
	shared:{
		...
		autoform: {
			...
			options: function()  {
				...
				return Meteor.users.find({_id: { $ne: Meteor.userId() }}).map( 
					function(us) { 
						return { 
							label: us.emails[0].address,
							value: us._id,
			...
			select2Options: function() {
				return {
					//tags: true,
					placeholder: "Choose users you want to share this tag and it's items with",
					//allowClear: true,
					tokenSeparators: [',', ';'],
...
SimpleSchema.messages({
   ...
   "notUnique" : "The given value is already taken, but needs to be unique. Please enter another value"
});
