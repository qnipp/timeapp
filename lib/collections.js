...
Schemas.Attributes = new SimpleSchema([{
	...
	type:{
		type: String,
		label: "Type of",
		allowedValues: ["text", "textarea", "number", "boolean-checkbox", "date", "datetime-local", "email", "url", "password", "select"],
		autoform: {
			options: [
	...
	ownedBy:{
		...
		custom: function() {
			...
			*/
			...
			if ( this.isSet && this.isUpdate && currentRow && currentRow.ownedBy && this.userId != currentRow.ownedBy) {
				return "unauthorized";
			}
	...
	attributes:{
		...
		autoform: {
			...
			select2Options: function() {
				return {
					//tags: true,
					placeholder: "Please select attributes for all items having this tag",
					//allowClear: true,
					tokenSeparators: [',', ';'],
					/*
					templateSelection: function(tag) {
