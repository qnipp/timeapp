...
Schemas.Items = new SimpleSchema([{
	...
	ownedBy:{
		...
		autoValue: function() {
			...
			// ich kann ein item nur an user weiter geben, - pff - 
			...
			*/
					console.log("createdBy autovalue: ");
					console.log(this.field('ownedBy'));
					
					// load current value in collection
					currentvalue = this.field("ownedBy");
					
					console.log('current value in collection: ' + currentvalue.value);
					
					if (!currentvalue.isSet && !this.value) {
						// if current value is not set, and no new value is given ..
						// .. set current user
						console.log('set current user id: ' + Meteor.userId());
				return Meteor.userId();
			} else if(currentvalue.isSet && currentvalue.value != Meteor.userId()) {
				// if current value is set, and is not the current user, unset given val
				console.log('current value is not current user: ' + currentvalue.value + ' != ' + Meteor.userId());
				this.unset();
			}
		},
		//denyUpdate: true,
