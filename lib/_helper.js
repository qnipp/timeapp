...
formatDuration = function(timeelapsed, shorten, cbMethod) {
  ...
  if (w > 0) {
    ...
    timeduration = timeduration.replace(/W(?![^\[]*\])/, w < 10 ? `0${w}` : w);
  } else {
...
getTimeslotValueFromTotals = function(totals, userid, timeslot) {
  if (userid) {
    const userindex = getUserIndexFromTotals(totals, userid);
    if (userindex == -1) {
      return 0;
    }
...
rgb2yuv = function(rgb) {
  const y = clamp(rgb.r * 0.299 + rgb.g * 0.587 + rgb.b * 0.114);
  const u = clamp(rgb.r * -0.16874 + rgb.g * -0.33126 + rgb.b * 0.5 + 128);
  const v = clamp(rgb.r * 0.5 + rgb.g * -0.41869 + rgb.b * -0.08131 + 128);
  return { y, u, v };
};
...
yuv2rgb = function(yuv) {
  ...
  const u = yuv.u;
  const v = yuv.v;
  const r = clamp(y + (v - 128) * 1.402);
  const g = clamp(y + (u - 128) * -0.34414 + (v - 128) * -0.71414);
  const b = clamp(y + (u - 128) * 1.772);
  return { r, g, b };
};
...
formatTagResult = function(state) {
  ...
  let tag = Tags.findOne({ _id: state.id });
  tag = loadTag(tag);

  return Blaze.toHTMLWithData(Template.taginputentry, { tag });
  /*
	var $state = $(
...
formatItemResult = function(state) {
  ...
  let item = Items.findOne({ _id: state.id });
  // item = loadItem(time.item, {all: true}, null);
  item = loadItem(item, false, null);

  return Blaze.toHTMLWithData(Template.iteminputentry, { item });
};
...
formatAttributeResult = function(state) {
  ...
  let attrib = Attributes.findOne({ _id: state.id });
