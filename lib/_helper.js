				...
				if(indexOfUser == -1) {
					...
					for (timeslot in CNF.timeslots) {
						...
						if(item.timeelapsed[founduserid][timeslot]) {
							...
							item.totals[indexOfUser][timeslot] += item.timeelapsed[founduserid][timeslot];
						}
					}
				}
				/*
...
formatDuration = function (timeelapsed, shorten, cbMethod) {
	...
	if(!timeelapsed || timeelapsed <= 0) {
		...
		if(w > 0) {
			...
			timeduration = timeduration.replace(/w/ig, '');
		}
...
jira_Auth = function(){
	/*
	 Meteor.call("fetchUrl", "https://jira.super-fi.net/rest/auth/1/session", 
		{ data: { "username": "andreas.pauschenwein", "password": "FILLME"} }, 
		function(error, result) { 
			console.log(error); 
			console.log(result); 
		}
...
jira_loadissue = function(){
	/*
	 * see: https://jira.super-fi.net/browse/INGWEB-911
	 Meteor.call("fetchUrl", "https://jira.super-fi.net/rest/api/2/issue/INGWEB-911", 
		{ headers: {"Cookie": "JSESSIONID=1D0DEB05CDB30E4A5265762052F1B654" }  }, 
		function(error, result) { 
			console.log(error); 
			console.log(result); 
